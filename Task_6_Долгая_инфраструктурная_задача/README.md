...
Хотя оно потом все равно слетит (почему-то у меня на альме слетает)
...
**OLD - 1.1.2.2. Создание админа**
<details>
  <summary>Описание шага. UPD: Делала при первом прохождении (вручную), сейчас делаю на этапе установки ОС</summary>
...
**1.1.2.4. Узнаю адрес машины для подключения по SSH**
```bash
ip addr
```
...
**1.1.2.5. Узнаю хостнейм**
При первом прохождении он некорректно настроился. При втором прохождении проверяю, все ли ок, надо ли менять:
```bash
hostname
```
...
**1.1.2.6. Проверяю, какой используется сетевой менеджер**
...
**OLD 1.1.2.7 - Настройка статического IP-адреса**
<details>
  <summary>Описание шага. UPD: Делала при первом прохождении (вручную), сейчас уношу в автоматизацию</summary>
...
**OLD 1.1.2.8 - Настройка хостнейма**
 <details>
  <summary>Описание шага. UPD: Делала при первом прохождении (вручную), сейчас уношу в автоматизацию </summary>
...
##### 1.2.1.3. Минимальный вариант установки ОС
 
![alt text](image-94.png)

##### 1.2.1.4. Ставлю время
Хотя оно потом все равно слетит (почему-то у меня на альме слетает)
 
![alt text](image-95.png)

##### 1.2.1.5. Задаю пароль руту и создаю админа
UPD: В первое прохождение админа создавала после установки ОС, сейчас решила сразу на этапе установки ОС:
 
![alt text](image-97.png)

#### 1.2.2. После установки ОС настраиваю машинку.
 
**1.2.2.1. Тип подключения к сети:** Bridge
 
**OLD - 1.2.2.2. Создание админа**
<details>
  <summary>Описание шага. UPD: Делала при первом прохождении (вручную), сейчас делаю на этапе установки ОС</summary>
...
**1.2.2.3. Проверяю, что SSH стоит (на Альме, как и на Центос должен стоять по умолчанию):**
```bash
sudo systemctl status sshd
```
Стоит:
...
**1.2.2.4. Узнаю адрес машины для подключения по SSH**
```bash
ip addr
```
(дальше все по SSH)
...
#### 1.3.1. Установка ОС
Здесь просто картинками по моментам, которые посчитала основными. В качестве локации выбрала РФ, для клавиатуры оставила англ.
...
>  Optional - т.к. работает по разному. При втором прохождении у меня все ок со временем, не понадобилось
...
> * `gitlab_shell_ssh_port`	- SSH-порт для клонирования репозиториев при использовании нестандартного порта (будет 2222, т.к. 22 используется для подключения по SSH)
...
>    * Порт 80 оставляю по умолчанию, т.к. эта ВМ выделена только под гитлаб-раннер, и других сервисов, которым может понадбиться порт 80 (например, nginx) не планируется. 
>    * Про порт 22 уже писала - он занят под SSH-доступ к хосту, поэтому меняю на 2222
> * `volumes` - связываю директории на сервере с директориями контейнера, базовый минимум в соответствии с оф. документацией:
>     * `/etc/gitlab` - под конфиги
>     * `/var/log/gitlab` - под логи
>     * `/var/opt/gitlab` - под данные
> * `$GITLAB_HOME` - уже задан ранее как /srv/gitlab в соответствии с оф. инструкцией
...
В инвентаре указываю две группы серверов:
```bash
