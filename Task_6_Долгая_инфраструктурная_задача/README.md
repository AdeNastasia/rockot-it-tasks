...
Но как я поняла, то в целом, если я задаю 512мб, реально доступно будет чуть меньше - и это нормально.
...
* /var - все оставшееся место, ext4 - логи. как я поняла, лучше сюда побольше места, т.к. сюда будет много писаться
...
UPD: во второе прохождение этого не делала
```bash
sudo adduser kaya
sudo passwd kaya
(дважды ввожу пароль)
...
При втором прохождении пошла тоже чуть по другому пути
...
UPD: неактуально во второе прохождение
**После установки создаю пользователя, чтобы не сидеть под рутом:**
```bash
sudo adduser kaya
sudo passwd kaya
