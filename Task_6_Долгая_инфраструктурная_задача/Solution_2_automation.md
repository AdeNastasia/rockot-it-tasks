...
### Этап 6. Обновление гитлаба
> Описание. Обновление гитлаба. Зачем - что это даст, описание плана и выполнение задачи.
 
#### 6.0. Зачем
Как я понимаю, самое основное - критичные обновления безопасности, если есть. Плюс появляются новые фичи. Если есть актуальные - значит надо обновиться. И еще совместимость. Если мы обновляем другое связанное ПО, то вероятно, может понадобиться обновление гитлаб-сервера.
 
#### 6.1. Бэкап
В интернетах вычитала, что сперва бэкап, а то мало ли что.

#### 6.2. Понять, что делать
Проверяю точную версию гитлаба
1) Смотрю по адресу http://local-gitlab.lan/api/v4/version (у меня 17.9.3):
 
![alt text](image-273.png)
 
2) Еще можно посмотреть в CLI:
```bash
...
3) Определяю корректный путь обновления гитлаба с помощью https://gitlab-com.gitlab.io/support/toolbox/upgrade-path:
 
![alt text](image-285.png)
 
Получаю три предупреждения:
1. PostgreSQL index corruption 
2. OpenSSL v3 Upgrade 
3. Multi-node secrets 
 
Еще получаю несколько кнопок:
 
![alt text](image-286.png)
 
**Прикинула такой план:**
1) Анализ уведомлений выше
2) Составить план
3) Сделать бэкап
4) Проверить, что хватит места на обновление (а сколько уместно закладывать - это пока не поняла)
5) Посмотреть все предупреждения из upgrade path - если что-то нужно сделать, то сделать
6) Поэтапно обновить, проверяя, что на каждой версии все поднялось и гитлаб работает: `17.9.3` -> `17.11.7` -> `18.2.5` -> `18.3.1`
 
**ВАЖНО! На каждом этапе перед апгрейдом учитывать миграции: после обновления сам гитлаб-сервер может продолжать работать, но в фоне у него еще могут бегать миграции БД.** 
 
> **Для инфо**
>  
> При обновлении гитлаба внутри базы PostgreSQL нужно изменить схему (таблицы, индексы, данные).
> Есть два типа миграций:
> * **Обычные миграции** - выполняются сразу при апгрейде, гитлаб-сервер не стартует, пока они не закончатся.
> * **Background migrations** - тяжелые миграции, которые гитлаб-сервер откладывает и выполняет постепенно в фоне, чтобы не положить базу на больших данных.
 
**Шаги, которые делаем один раз перед первым обновлением**
 
**Шаг 0 - анализ уведомлений из upgrade path**
 
У меня было три алерта:
 
![alt text](image-285.png)
 
1) Первый, как я понимаю, про обновление ОС и библиотеки. Я интерпретировала так, что надо:
- уточнить, есть ли смысл обновлять гитлаб на этой машине, планируется ли там обновление ОС
- если есть какая-то документация к серверу, оставить там эту пометку
 
У меня на ВМ можно обновлять, получается

2) Второй алерт про то, что если есть внешние интеграции, где важен TLS/SSL - погрузиться подробнее и посмотреть, что за изменения в openssl, будут ли они нам что-то ломать, что нужно обновить. У меня не используется на ВМ

3) Третий алерт - про кластер. А у меня одна ВМ, можно обновлять

Делаю такой вывод:
Если прод - собрать все инфо, подробно разобраться, что затронет обновление. Уточнить у команды, ок ли нам, будем ли обновлять. Если будем, то прописать доп. шаги к обновлению по этой теме.
У меня все учебное, все можно обновлять 

**Шаг 1 - остановка работы**
 
По аналогии с этапом по миграции при работе с реальными серверами:
* либо убрать сервер из nginx, чтобы он временно был недоступен (чтобы не появилось новых изменений, не вошедших в бэкап)
* либо использовать спец. опцию на платном гитлабе (Maintenance mode), если она реально существует (я не видела, у меня `ce`)
...
Проверяю, что есть место:
```bash
df -h /srv/gitlab
```
![alt text](image-288.png)
 
**Шаги цикла обновления (для каждой версии)**
 
**Шаг 1 - обновление компоуза**
 
Перехожу в директорию к файлу компоуза:
```bash
cd /srv/gitlab/
```
...
В строчке `image: gitlab/gitlab-ce:17.9.3-ce.0` меняю версию на каждой итерации: в первой на 17.11.7, во второй - 18.2.5, в третьей - 18.3.1 
 
**Шаг 2 - опустить/поднять**
 
Перед стартом нового контейнера опускаю текущий:
```bash
docker compose down
```
...
docker compose up -d
``` 
> p.s. я подключаюсь через wsl, и сейчас сессия прерывается. поэтому временно переключилась на саму ВМ и делаю там
> ![alt text](image-289.png)
 
**Шаг 3 - проверки**
 
Жду и проверяю, что все сервисы гитлаба запустились:
```bash
docker ps
```
и
...
Проверяю, что версия гитлаба обновилась до нужной:
```bash
docker exec -it gitlab gitlab-rake gitlab:env:info 
```
И правда:
...
Запускаю встроенную самопроверку гитлаба на проверку ошибок после апгрейда:
```bash
docker exec -it gitlab gitlab-rake gitlab:check SANITIZE=true
```
...
Смотрю, как дела, в интерфейсе (проверила только проекты, все на месте):
 
![alt text](image-293.png)
 
Перед будущим обновлением проверяю background migrations:
* либо через интерфейс (чтобы все были `finished`) http://local-gitlab.lan/admin/background_migrations
* либо через консоль (что везде `finalized`) `docker exec -it gitlab gitlab-rake gitlab:background_migrations:status`
 
Я пока посмотрела через консоль:
 
![alt text](image-304.png)
 
> Далее все те же шаги только смена версии в компоузе. Отдельно вынесла проверки
 
**Проверки после второй итерации обновления (на 18.2.5)**
 
Работоспособность контейнера и сервисов:
...
Интерфейс (проекты на месте) - забыла скрин, но они на месте.
 
background migrations - тоже без скрина, но они на месте
 
**Проверки после третьей итерации обновления (на 18.3.1)**
...
Интерфейс - проекты на месте
 
background_migrations через интерфейс - http://local-gitlab.lan/admin/background_migrations, а не через консоль (неудивительно, что так удобнее), все в finished
 
![alt text](image-303.png)
 
На этом этапе я еще запустила самый простой пайпалан, который ранее 100% работал. Сейчас тоже работает:
 
![alt text](image-305.png)
 
Единственное, на ВМ гитлаб-сервера время слетело (т.к. на скрине написано, что пайплайн выполнился 3 часа назад), выполнила `sudo systemctl restart chronyd`.
 
У уже прошедших пайплайнов время осталось прежним. У нового стало актуальным:
