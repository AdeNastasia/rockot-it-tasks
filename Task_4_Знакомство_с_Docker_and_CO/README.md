...
#### Вопрос 1.10. 
✅ Готово! Разбираюсь с переменными
...
#### Вопрос 2.6.
Нам надо прописать в `.env` переменные, которые мы будем использовать и задать им значения по умолчанию. Здесь нам не надо указывать `ARG` или `ENV`, так как мы просто задаем переменные, по сути, какие хотим:
```console
...
Чтобы их использовать в Dockerfile как ENV мы прописываем в Dockerfile ENV переменные (тут уже не получится задать какое угодно имя, так как это переменные окружения, то это должны быть реальные переменные окружения в будущем контейнере). И далее в качестве значения нам надо подскавить те переменные, которые мы указали в `.env` с помощью специального синтаксиса, который поставит сами значения переменных: ${ИМЯ_ПЕРЕМЕННОЙ}
```console
ENV APACHE_RUN_GROUP=${APACHE_RUN_GROUP}
ENV APACHE_LOG_DIR=${APACHE_LOG_DIR}
ENV APACHE_RUN_DIR=${PACHE_RUN_DIR}
ENV APACHE_PID_FILE=${APACHE_PID_FILE}
```

Благодаря тому, что в `.env` мы можем задавать любые имена переменным, то мы можем создать две похожие переменные, но явно указать, что они для разных конетйнеров (если этого требует ситуация). Например:
```console
APACHE_LOG_DIR_FOR_DEBIAN=/var/log/apache2
APACHE_LOG_DIR_FOR_ALMALINUX=/var/log/httpd
```

И значение каждой из переменных мы сможем без проблем установить как значение для переменной окружения в контейнере:
`ENV APACHE_LOG_DIR=${APACHE_LOG_DIR_FOR_DEBIAN}` 
 или 
`ENV APACHE_LOG_DIR=${APACHE_LOG_DIR_FOR_ALMALINUX}`
